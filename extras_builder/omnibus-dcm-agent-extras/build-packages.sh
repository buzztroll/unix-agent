...
DCM_AGENT_EXTRAS_VERSION=`grep SEARCH_TOKEN omnibus-dcm-agent-extras/config/projects/dcm-agent-extras.rb | head -n 1 | awk '{ print $2 }' | sed 's/"//g'`
...
docker_envs="-e DCM_AGENT_EXTRAS_VERSION=$DCM_AGENT_EXTRAS_VERSION -e AWS_ACCESS_KEY=$AWS_ACCESS_KEY -e AWS_SECRET_KEY=$AWS_SECRET_KEY -e DCM_AWS_EXTRAS_BUCKET=$DCM_AWS_EXTRAS_BUCKET"
cp -r * $working_dir
